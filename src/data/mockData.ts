...
function generateCoreMonthlyStats(): CoreMonthlyStats[] {
  const months = generateMonths();
  return months.map((month) => ({
    month,
    visits: randomInt(8000, 25000),
    tours: randomInt(150, 400),
  }));
}
...
export function generateMockData(): DashboardData {
  ...
  const coreMonthlyStats = generateCoreMonthlyStats();
  const totalVisits = coreMonthlyStats.reduce((sum, m) => sum + m.visits, 0);
  const totalTours = coreMonthlyStats.reduce((sum, m) => sum + m.tours, 0);
