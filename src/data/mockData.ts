...
const GUIDE_NAMES = [
  '张明辉', '李小红', '王建国', '赵雨晴', '刘子轩', '陈雨桐',
  '杨浩然', '周佳怡', '吴俊杰', '郑思琪', '孙文博', '马晓燕',
  '朱明宇', '胡雅婷', '林浩然', '何雨欣', '高志远', '罗梦琪',
  '梁宇轩', '宋雨彤', '唐浩然', '许佳怡', '韩俊杰', '冯思琪',
];
...
function generateSchoolData(): { rankings: SchoolData[]; details: Record<string, SchoolDetail> } {
  ...
  SCHOOLS.forEach((school, idx) => {
    ...
    const teams = Array.from({ length: teamCount }, () => ({
      id: generateId(),
      date: months[randomInt(0, months.length - 1)] + `-${String(randomInt(1, 28)).padStart(2, '0')}`,
      studentCount: randomInt(25, 55),
      theme: TEAM_THEMES[randomInt(0, TEAM_THEMES.length - 1)],
    })).sort((a, b) => b.date.localeCompare(a.date));

    const guides: GuideDetail[] = Array.from({ length: guideCount }, (_, gi) => ({
      id: `guide-${id}-${gi}`,
      name: GUIDE_NAMES[(idx * 3 + gi) % GUIDE_NAMES.length],
      school: school.name,
      grade: GRADES[randomInt(0, GRADES.length - 1)],
      serviceHours: randomInt(20, Math.floor(guideServiceHours / guideCount) + 10),
      joinDate: months[randomInt(0, 6)] + `-${String(randomInt(1, 28)).padStart(2, '0')}`,
    }));
...
export function generateMockData(): DashboardData {
  ...
  const totalVisits = rankings.reduce((sum, s) => sum + s.studentCount, 0) + randomInt(50000, 150000);
  const totalTours = randomInt(2000, 5000);
