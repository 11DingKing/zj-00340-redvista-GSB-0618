...
function generateMonthlyCoreStats(): CoreStatsMonthly[] {
  const months = generateMonths();
  return months.map((month) => {
    ...
    return {
      month,
      totalVisits: Math.round(randomInt(8000, 20000) * seasonFactor),
      totalTours: Math.round(randomInt(150, 400) * seasonFactor),
    };
  });
}
...
export function generateMockData(): DashboardData {
  ...
  const monthlyCoreStats = generateMonthlyCoreStats();
  const totalVisits = monthlyCoreStats.reduce(
    (sum, m) => sum + m.totalVisits,
    0,
  );
  const totalTours = monthlyCoreStats.reduce((sum, m) => sum + m.totalTours, 0);
